...
-- as if the tree is an `AVL`; the verification of the `AVL` restrictions is performed after the deletion.
...
-- in a tree 't' without checking any structural invariant (key ordering or height balance).
-- The deletion is defined at the value level and the type level, and is performed
-- as if the tree is a `AVL`; the verification of the `AVL` restrictions is performed after the deletion.
